...
(3, 1, '2024-11-17', NULL), -- Charlie borrowed The Secret Garden (Not returned yet, recent - for Q48)
...
select b.Title, bo.ReturnDate
from Books b
join BorrowRecords bo
on b.BookID = bo.BookID
where bo.ReturnDate IS NULL

--5. Show all members who have borrowed at least one book.
select M.MemberID, M.FullName, count(Bo.BorrowID) as TotalBorrowed
from Members M
join BorrowRecords Bo
...
select b.title, b.author, bo.BorrowDate, bo.ReturnDate
from Books b
join BorrowRecords bo
...
select b.Genre,sum(b.AvailableCopies) as TotalAvailableBooks
from Books b
group by b.Genre
